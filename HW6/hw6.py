...
class ConvNet(nn.Module):
    ...
    def __init__(self, kernel_size=3, length=10, out_chan=32, bias=True):
        ...
        self.conv = nn.Conv1d(1, out_chan, kernel_size=kernel_size, bias=bias)
        self.transpose_conv = nn.ConvTranspose1d(out_chan, 1, kernel_size=kernel_size, bias=bias)
        self.relu = nn.ReLU()
...
class Attention(nn.Module):
    ...
    def compute_new_values(self, q, k, v):
        ...
        attentions = F.softmax(energy, dim=2)
...
if __name__ == "__main__":
    ...
    if model_type == 'Convolution':
        ...
        #model10 = ConvNet(length=length, kernel_size=kernel_ten).to(device) 
    ...
    #criterion10 = nn.MSELoss(reduction="none")

    #ker3 = []
    #ker10 = []
    for epoch in range(25):
        train(model, epoch, optimizer, criterion, trainloader)
        #train3 = train(model, epoch, optimizer, criterion, trainloader)
        #train10 = train(model10, epoch, optimizer10, criterion10, trainloader)
        #ker3.append(train3)
        #ker10.append(train10)
